            ...
            for(Attack attack : attacks) {
                if(moveset.size() < 4) {
                    ...
                    if(!event.isCancelled()) {
                        ...
                        Bukkit.getServer().getPlayer(ranch.getOwnerUUID()).sendMessage(MessageUtils.parse(
                                String.format("&7Your &e%s &7leveled up, and ended up learning &e%s!", this.getDelegate().getSpecies().getPokemonName(), attack.baseAttack.getTranslatedName().getFormattedText()),
                                false
                        ));
                    }
                } else {
                    ...
                    if(!event.isCancelled()) {
                        ...
                        Bukkit.getServer().getPlayer(ranch.getOwnerUUID()).sendMessage(MessageUtils.parse(
                                String.format("&7Your &e%s &7leveled up, and ended up learning &e%s &7in replacement of &e%s!",
                                        ...
                                        attack.baseAttack.getTranslatedName().getFormattedText(),
                                        old.baseAttack.getTranslatedName().getFormattedText()),
                                false
                        ));
                    }
