...
public class PokemonTokens implements TokenHolder {
	...
	static {
		...
		tokens.put("gained_lvls", (p, v, m) -> {
			GenerationsDaycarePokemonWrapper pokemon = getWrapperFromVariableIfExists(m);
			if (pokemon == null) {
				return Optional.empty();
			}
		...
		tokens.put("calced_lvl", (p, v, m) -> {
			...
			return Optional.of(Text.of(pokemon.getDelegate().getLvl().getLevel() + pokemon.getGainedLevels()));
		});
	...
	private static EntityPixelmon getPokemonFromVariableIfExists(Map<String, Object> m) {
		return m.values().stream().filter(val -> val instanceof EntityPixelmon).map(val -> (EntityPixelmon) val).findAny().orElseGet(() -> {
			GenerationsDaycarePokemonWrapper wrapper = getWrapperFromVariableIfExists(m);
			if (wrapper != null) {
	...
	private static GenerationsDaycarePokemonWrapper getWrapperFromVariableIfExists(Map<String, Object> m) {
		return m.values().stream().filter(val -> val instanceof GenerationsDaycarePokemonWrapper).map(val -> (GenerationsDaycarePokemonWrapper) val).findAny().orElse(null);
	}
